    ...
    for (lineIndex in lines.indices) {
        ...
        for (charIndex in line.indices) {
            ...
            try {
                    ...
                    isSymbols = { symbol ->
                        ...
                        symbol.symbolType(
                            ...
                            isBoundary = { boundary ->
                                ...
                                // 写入结果集
                                resultWordType.addE(boundaries[0].wordType)
                            ...
                            isOperator = { operator ->
                                ...
                                if (resultWordType.isNotEmpty() && resultWordType.last().word.isNotEmpty()) {
                                    ...
                                    if (binaryOperatorMatch.isNotEmpty()) {
                                        ...
                                        // 将匹配成功的双目运算符加入结果集
                                        resultWordType.addE(binaryOperatorMatch[0].wordType)
                                        return@symbolType
                                    } else {
                                ...
                                // 写入结果集
                                resultWordType.addE(operators[0].wordType)
                            ...
                            isOther = { otherChar ->
                                ...
                                if (buffer.contains("\"") || buffer.contains("'")) {
                                    ...
                                    throw AnalysisException("发现未定义符号")
                                }
                            }
                        )
                    }
                ...
                // 抛出异常，并附带出错的行和列
                throw AnalysisException(e.message.toString(), lines, lineIndex, charIndex)
            }
        }
    }
